		...
		.get(
				...
				function(req, res) {
					...
					function loadData(data) {
						...
						return q.all([
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-category.json'), 'utf-8')
										.then(JSON.parse).then(setPlainTagCategories),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-al.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-cc.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-ccs.json'), 'utf-8')
										.then(JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-ge.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-gr.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-in.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-it.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-na.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-pe.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-ss.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-up.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-yr.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-ds.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-sy.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-co.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-se.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
								q.nfbind(fs.readFile)(
										path.join(__dirname, '../data/tag-sp.json'), 'utf-8').then(
										JSON.parse).then(addPlainTags),
					...
					function updateTags(data) {
						/**
						 * dependency = tag collection
						 */

						function insertTagDependency(dependency) {
							console.log("Inserting tag dependency");
						...
						/**
						 * update tag dependencies results.result = dependency collection
						 */

						function updateTagDependencies(plainTag) {
							return function(results) {
								console.log("Updating tag dependencies");
								tagDictionary[plainTag.category][plainTag.name]._dependencies = results.result;
								return tagDictionary[plainTag.category][plainTag.name].save();
							}
						}
						console.log("Updating tags");
						return q.all(_.map(plainTags, function(plainTag) {
							return q.all(_.map(plainTag.tags, insertTagDependency))//
							.then(updateTagDependencies(plainTag))
						}));
					}
