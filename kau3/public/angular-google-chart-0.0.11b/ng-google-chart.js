...
/* global angular */
(function() {
	angular.module('googlechart', []).run(registerResizeEvent);
...
(function() {
	...
	function formatManagerFactory() {
		...
		function FormatManager() {
			...
			self.iFormats = {}; // Holds instances of formats (ie.
													// self.iFormats.date[0] = new
													// google.visualization.DateFormat(params))
				...
				for (formatType in tFormats) {
					if (tFormats.hasOwnProperty(formatType)) {
						for (i = 0; i < self.iFormats[formatType].length; i++) {
							if (tFormats[formatType][i].columnNum < dataTable
									.getNumberOfColumns()) {
								self.iFormats[formatType][i].format(dataTable,
										tFormats[formatType][i].columnNum);
							}
						}
					}
				}
			...
			function applyFormat(formatType, FormatClass, tFormats) {
				...
				if (angular.isArray(tFormats[formatType])) {
					...
					if (!angular.equals(tFormats[formatType],
							oldFormatTemplates[formatType])) {
			...
			function applyFormats(dataTable, tFormats, customFormatters) {
				...
				for (formatType in tFormats) {
					if (tFormats.hasOwnProperty(formatType)) {
						...
						if (formatType === 'arrow' || formatType === 'bar'
								|| formatType === 'color') {
							requiresHtml = true;
						}
					}
			...
			function instantiateColorFormatters(tFormats) {
				...
				for (t = 0; t < tFormats[formatType].length; t++) {
					...
					for (i = 0; i < tFormats[formatType][t].formats.length; i++) {
						...
						if (typeof (data.fromBgColor) !== 'undefined'
								&& typeof (data.toBgColor) !== 'undefined') {
							colorFormat.addGradientRange(data.from, data.to, data.color,
									data.fromBgColor, data.toBgColor);
						} else {
							colorFormat
									.addRange(data.from, data.to, data.color, data.bgcolor);
						}
					}
			...
			function getFormatClass(formatType, customFormatters) {
				var className = formatType.charAt(0).toUpperCase()
						+ formatType.slice(1).toLowerCase() + "Format";
				if (google.visualization.hasOwnProperty(className)) {
					return google.visualization[className];
				} else if (angular.isDefined(customFormatters)
						&& customFormatters.hasOwnProperty(formatType)) {
...
/* jshint -W072 */
(function() {
	angular.module('googlechart').directive('googleChart', googleChartDirective);

	googleChartDirective.$inject = [ '$timeout', '$window', '$rootScope',
	...
	function googleChartDirective($timeout, $window, $rootScope, googleChartApiPromise) {
		...
		GoogleChartController.$inject = [ '$scope', '$element', '$attrs',
				'$injector', 'FormatManager' ];

		function GoogleChartController($scope, $element, $attrs, $injector, FormatManager) {
			var self = this;
			var resizeHandler, wrapperListeners = {}, formatManager, chartListeners = {}, innerVisualization = null;
			...
			function handleError(args) {
				var error = args[0];
				console.log("Chart not displayed due to error: '" + error.message
						+ "' Full error object follows.");
				console.log(error);
			}
			...
			function init() {
				/*
				 * Watches, to refresh the chart when its data, formatters, options,
				 * view, or type change. All other values intentionally disregarded to
				 * avoid double calls to the draw function. Please avoid making changes
				 * to these objects directly from this directive.
				 */
				$scope.$watch(watchValue, watchHandler, true); // true is for deep
																												// object equality
																												// checking
			...
			function setupAndDraw() {
				if (typeof (self.chartWrapper) === 'undefined') {
					...
					self.chartWrapper = new google.visualization.ChartWrapper(
							chartWrapperArgs);
			...
			function registerListener(listenerCollection, eventName, listenerFn, listenerObject) {
				...
				if (angular.isDefined(listenerCollection)
						&& angular.isObject(listenerCollection)) {
					...
					return function() {
						if (angular.isDefined(listenerWrapper.googleListenerHandle)) {
							google.visualization.events
									.removeListener(listenerWrapper.googleListenerHandle);
						}
						var fnIndex = listenerCollection[eventName]
								.indexOf(listenerWrapper);
			...
			function registerListenersWithGoogle(eventSource, listenerCollection) {
				for ( var eventName in listenerCollection) {
					if (listenerCollection.hasOwnProperty(eventName)
							&& angular.isArray(listenerCollection[eventName])) {
						for (var fnIterator = 0; fnIterator < listenerCollection[eventName].length; fnIterator++) {
							if (angular.isFunction(listenerCollection[eventName][fnIterator])) {
								listenerCollection[eventName][fnIterator].googleListenerHandle = google.visualization.events
										.addListener(eventSource, eventName,
												listenerCollection[eventName][fnIterator]);
							}
						}
					}
				}
			...
			function registerChartListener(eventName, listenerFn, listenerObject) {
				return registerListener(chartListeners, eventName, listenerFn,
						listenerObject);
			}

			function registerWrapperListener(eventName, listenerFn, listenerObject) {
				return registerListener(wrapperListeners, eventName, listenerFn,
						listenerObject);
			}
		}
...
(function() {
	angular.module('googlechart').factory('googleChartApiPromise',
			googleChartApiPromiseFactory);

	googleChartApiPromiseFactory.$inject = [ '$rootScope', '$q',
			'googleChartApiConfig', 'googleJsapiUrl' ];
...
/* global angular */
(function() {
	angular.module('googlechart').provider('googleJsapiUrl',
...
/* global angular */
(function() {
	angular.module('googlechart').directive('onError', onErrorDirective);
...
/* global angular */
(function() {
	angular.module('googlechart').directive('onReady', onReadyDirective);
...
/* global angular */
(function() {
	angular.module('googlechart').directive('onSelect', onSelectDirective);
		...
		return {
			...
			link : function(scope, element, attrs, googleChartController) {
				...
				function callback(chartWrapper, chart) {
					...
					// This is for backwards compatibility for people using 'selectedItem'
					// that only wanted the first selection.
